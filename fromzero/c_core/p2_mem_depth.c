// 记忆深度实验: 因果线性注意力(无位置编码) 能记住周期 k 多深?
// 任务: 周期 k 随机模式 S[t]=S[t-k], 位置 t 预测 S[t+1]=S[t+1-k]
//   当前 token 与目标无一一对应(k≥2 时不可由 bigram 决定), 必须靠状态记忆
// 对照: 若 k=1 能学、k≥2 学不会 → 证明无位置编码的 query 无法区分"几步前"
#include <stdio.h>
#include <math.h>
#include <stdint.h>
...
int main(void){
    printf("== 记忆深度实验: 因果线性注意力(无位置编码) ==\n");
    printf("任务: 周期K随机模式, 位置t预测S[t+1]=S[t+1-K] (需状态记忆, 非bigram)\n\n");
    for(int k=1;k<=6;k++){
        K=k;
